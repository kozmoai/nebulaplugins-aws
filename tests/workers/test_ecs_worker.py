...
from nebula_aws.credentials import _get_client_cached
from nebula_aws.workers.ecs_worker import (
...
async def test_default(aws_credentials: AwsCredentials, flow_run: FlowRun):
    ...
    assert task == {
        ...
        #       reasonable values
        "containers": [{"exitCode": 0, "name": "nebula"}],
        "desiredStatus": "STOPPED",
        "lastStatus": "STOPPED",
        "launchType": "FARGATE",
        "overrides": {
            "containerOverrides": [
                {"name": "nebula", "environment": [], "command": ["echo", "test"]}
            ]
        },
        "startedBy": ANY,
        "tags": [],
...
async def test_image(aws_credentials: AwsCredentials, flow_run: FlowRun):
    configuration = await construct_configuration(
        aws_credentials=aws_credentials, image="kozmoai/nebula-dev:main-python3.9"
    )
    ...
    assert task_definition["containerDefinitions"] == [
        {
            "name": ECS_DEFAULT_CONTAINER_NAME,
            "image": "kozmoai/nebula-dev:main-python3.9",
            "cpu": 0,
            "memory": 0,
            "portMappings": [],
            "essential": True,
...
async def test_environment_variables(
    ...
    task = describe_task(ecs_client, task_arn)
    task_definition = describe_task_definition(ecs_client, task)
    nebula_container_definition = _get_container(
        task_definition["containerDefinitions"], ECS_DEFAULT_CONTAINER_NAME
    )
    ...
    nebula_container_overrides = _get_container(
        task["overrides"]["containerOverrides"], ECS_DEFAULT_CONTAINER_NAME
    )
...
async def test_user_defined_container_image_in_task_definition_template(
    ...
    nebula_container = _get_container(
        task_definition["containerDefinitions"], ECS_DEFAULT_CONTAINER_NAME
    )
    assert (
...
async def test_user_defined_environment_variables_in_task_definition_template(
    ...
    assert nebula_container_overrides.get("environment") == [
        {"name": "FOO", "value": "BAR"},
...
async def test_user_defined_environment_variables_in_task_run_request_template(
    ...
    assert (
        nebula_container_definition["environment"] == []
...
async def test_mask_sensitive_env_values():
    task_run_request = {
        "overrides": {
            "containerOverrides": [
                {
                    "environment": [
                        {"name": "NEBULA_API_KEY", "value": "SeNsItiVe VaLuE"},
                        {"name": "NEBULA_API_URL", "value": "NORMAL_VALUE"},
                    ]
                }
            ]
        }
    ...
    res = mask_sensitive_env_values(task_run_request, ["NEBULA_API_KEY"], 3, "***")
    assert (
