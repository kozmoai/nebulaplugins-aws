...
from nebula.server.schemas.core import Deployment, Flow, FlowRun
...
from nebula_aws.workers.ecs_worker import ECSWorker
...
def test_ecs_task_emits_deprecation_warning():
    with pytest.warns(
        ...
        match=(
            "nebula_aws.ecs.ECSTask has been deprecated."
...
- cpu: 1024
  image: kozmoai/nebula:2.1.0-python3.8
  memory: 2048
  name: nebula
family: nebula
"""
...
def patch_describe_tasks_add_nebula_container(describe_tasks, *args, **kwargs):
    """
    Adds the minimal nebula container to moto's task description.
    """
    result = describe_tasks(*args, **kwargs)
    for task in result:
        if not task.containers:
            task.containers = []
        if not get_nebula_container(task.containers):
            task.containers.append({"name": "nebula"})
    return result
            ...
            with mock_logs():
                ...
                inject_moto_patches(
                    ecs,
                    {
                        ...
                        "describe_tasks": [patch_describe_tasks_add_nebula_container],
...
async def test_cpu_and_memory(aws_credentials, launch_type: str, cpu: int, memory: int):
    ...
    # And as overrides for the Nebula container
...
async def test_container_command(aws_credentials, launch_type: str):
    ...
    assert container_overrides["command"] == ["nebula", "version"]
...
async def test_environment_variables(aws_credentials):
    ...
    task = describe_task(ecs_client, task_arn)
    task_definition = describe_task_definition(ecs_client, task)
    nebula_container_definition = get_nebula_container(
        task_definition["containerDefinitions"]
    )
    assert not nebula_container_definition[
        "environment"
    ], "Variables should not be passed until runtime"

    nebula_container_overrides = get_nebula_container(
    ...
    assert nebula_container_overrides.get("environment") == expected
...
async def test_container_command_from_task_definition(aws_credentials):
    task = ECSTask(
        ...
        auto_deregister_task_definition=False,
        task_definition={
            "containerDefinitions": [{"name": "nebula", "command": ["echo", "hello"]}]
        },
        command=[],
    )
    print(task.preview())
...
async def test_extra_containers_in_task_definition(aws_credentials):
    ...
    nebula_container = get_nebula_container(task_definition["containerDefinitions"])
    assert nebula_container is not None, "The nebula container should be added"
    assert (
        nebula_container["image"] == "test"
    ), "The nebula container should use the image field"
    ...
    nebula_container_overrides = get_nebula_container(container_overrides)
    assert (
        user_container_overrides is None
    ), "The user container should not be included in overrides"
    assert (
        nebula_container_overrides
    ), "The nebula container should have overrides still"


@pytest.mark.usefixtures("ecs_mocks")
async def test_nebula_container_in_task_definition(aws_credentials):
    task = ECSTask(
        aws_credentials=aws_credentials,
        auto_deregister_task_definition=False,
        task_definition={
    ...
    assert nebula_container["command"] == [
        "should",
        "be",
        "gone",
...
async def test_default_image_in_task_definition(aws_credentials):
    ...
    nebula_container = get_nebula_container(task_definition["containerDefinitions"])
    assert (
        nebula_container["image"] == "use-this-image"
...
async def test_image_overrides_task_definition(aws_credentials):
    ...
    nebula_container = get_nebula_container(task_definition["containerDefinitions"])
    assert (
        nebula_container["image"] == "override-image"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {},
        # Task definition with nebula container but no image
        {
            "containerDefinitions": [
                {
                    "name": "nebula",
...
async def test_default_image(aws_credentials, task_definition):
    ...
    # The image on the block is inferred from Nebula/Python versions
    assert task.image == get_nebula_image_name()
    ...
    nebula_container = get_nebula_container(task_definition["containerDefinitions"])
    assert (
        nebula_container["image"] == get_nebula_image_name()
    ), "The image should be the default Nebula tag"
...
async def test_environment_variables_in_task_definition(aws_credentials):
    ...
    assert nebula_container_definition["environment"] == [
        {"name": "BAR", "value": "FOO"},
        {"name": "OVERRIDE", "value": "OLD"},
    ]
    ...
    assert nebula_container_overrides.get("environment") == expected_base + [
        {"name": "FOO", "value": "BAR"},
...
async def test_unset_environment_variables_in_task_definition(aws_credentials):
    ...
    assert (
        nebula_container_overrides.get("environment") == expected_base
...
async def test_configure_cloudwatch_logging(aws_credentials):
    ...
    for container in task_definition["containerDefinitions"]:
        if container["name"] == "nebula":
            # Assert that the 'nebula' container has logging configured
...
async def test_stream_output(aws_credentials, caplog):
    ...
    async def write_fake_log(task_arn):
        ...
        stream_name = f"test-prefix/nebula/{task_arn.rsplit('/')[-1]}"
    ...
    logs_client = session.client("logs")
    streams = logs_client.describe_log_streams(logGroupName="nebula")["logStreams"]
...
async def test_cloudwatch_log_options(aws_credentials):
    ...
    for container in task_definition["containerDefinitions"]:
        if container["name"] == "nebula":
            # Assert that the 'nebula' container has logging configured with user
            # provided options
            assert container["logConfiguration"] == {
                "logDriver": "awslogs",
                "options": {
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({}, {}, "nebula"),
        # Only flow
        ({}, {"flow": Flow(name="foo")}, "nebula__foo"),
        # Only deployment
        (
            {},
            {"deployment": Deployment.construct(name="foo")},
            "nebula__unknown-flow__foo",
        ),
        # Flow and deployment
        (
            {},
...
def base_job_template_with_task_arn(default_base_job_template, aws_credentials):
    ...
    base_job_template_with_task_arn["job_configuration"]["task_definition"] = {
        "containerDefinitions": [
            {"image": "docker.io/my_image:latest", "name": "nebula-job"}
        ],
        "cpu": "2048",
        "family": "test-family",
        "memory": "2024",
