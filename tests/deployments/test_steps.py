...
from nebula_aws.deployments.steps import get_s3_client, pull_from_s3, push_to_s3
...
def tmp_files(tmp_path: Path):
    ...
    (tmp_path / ".nebulaignore").write_text("""
    testdir1/*
    .nebulaignore
    """)
...
def test_custom_credentials_and_client_parameters(s3_setup, tmp_files):
    ...
    for file in tmp_files.iterdir():
        if file.is_file() and file.name != ".nebulaignore":
...
def test_without_nebulaignore_file(s3_setup, tmp_files: Path, mock_aws_credentials):
    s3, bucket_name = s3_setup
    folder = "my-project"

    # Remove the .nebulaignore file
    (tmp_files / ".nebulaignore").unlink()

    os.chdir(tmp_files)

    # Test push_to_s3 without .nebulaignore file
    push_to_s3(bucket_name, folder)

    # Test pull_from_s3 without .nebulaignore file
    tmp_path = tmp_files / "test_pull"
    tmp_path.mkdir(parents=True, exist_ok=True)
    os.chdir(tmp_path)
...
def test_nebulaignore_with_comments_and_empty_lines(
    s3_setup, tmp_files: Path, mock_aws_credentials
):
    s3, bucket_name = s3_setup
    folder = "my-project"

    # Update the .nebulaignore file with comments and empty lines
    (tmp_files / ".nebulaignore").write_text("""
        # This is a comment
        testdir1/*
