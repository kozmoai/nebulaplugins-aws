...
For upgrade instructions, see https://docs.nebula.io/latest/guides/upgrade-guide-agents-to-workers/.
...
from nebula.blocks.core import BlockNotSavedError
from nebula.exceptions import InfrastructureNotAvailable, InfrastructureNotFound
from nebula.infrastructure.base import Infrastructure, InfrastructureResult
from nebula.utilities.asyncutils import run_sync_in_worker_thread, sync_compatible
from nebula.utilities.dockerutils import get_nebula_image_name
from nebula.utilities.pydantic import JsonPatch
from pydantic import VERSION as PYDANTIC_VERSION

from nebula_aws.utilities import assemble_document_for_patches
...
from nebula_aws.workers.ecs_worker import _TAG_REGEX, ECSWorker
...
if TYPE_CHECKING:
    from nebula.client.schemas import FlowRun
    from nebula.server.schemas.core import Deployment, Flow
...
NEBULA_ECS_CONTAINER_NAME = "nebula"
ECS_DEFAULT_CPU = 1024
ECS_DEFAULT_MEMORY = 2048
ECS_DEFAULT_FAMILY = "nebula"
...
def get_nebula_container(containers: List[dict]) -> Optional[dict]:
    """
    Extract the Nebula container from a list of containers or container definitions.
    If not found, `None` is returned.
    """
    return get_container(containers, NEBULA_ECS_CONTAINER_NAME)
...
@deprecated_class(
    ...
    help=(
        ...
        " https://docs.nebula.io/latest/guides/upgrade-guide-agents-to-workers/."
    ),
)
class ECSTask(Infrastructure):
    ...
    Attributes:
        ...
        task_definition_arn: An optional identifier for an existing task definition
            ...
            Cannot be used with task_definition. If not provided, Nebula will
            generate and register a minimal task definition.
        task_definition: An optional ECS task definition to use. Nebula may set
            defaults or override fields on this task definition to match other
            ECSTask fields. Cannot be used with task_definition_arn.
            If not provided, Nebula will generate and register
        ...
        image: An optional image to use for the Nebula container in the task.
            If this value is not null, it will override the value in the task
            definition. This value defaults to a Nebula base image matching
            your local versions.
        ...
        execution_role_arn: An execution role to use for the task.
            ...
            from the container.
        configure_cloudwatch_logs: A boolean that controls if the Nebula
        ...
        stream_output: A boolean indicating whether logs will be
            streamed from the Nebula container to the local console.
        ...
        env: A dictionary of environment variables to provide to
            ...
            Nebula container at task runtime.
    ...
    _documentation_url = (
        "https://kozmoai.github.io/nebulaplugins-aws/ecs/#nebula_aws.ecs.ECSTask"  # noqa
    )
    ...
    task_definition: Optional[dict] = Field(
        default=None,
        description=(
            "An ECS task definition to use. Nebula may set defaults or override "
            "fields on this task definition to match other `ECSTask` fields. "
            "Cannot be used with `task_definition_arn`. If not provided, Nebula will "
            "generate and register a minimal task definition."
        ),
    )
    ...
    image: Optional[str] = Field(
        default=None,
        description=(
            "The image to use for the Nebula container in the task. If this value is "
            "not null, it will override the value in the task definition. This value "
            "defaults to a Nebula base image matching your local versions."
        ),
    )
    ...
    configure_cloudwatch_logs: bool = Field(
        default=None,
        description=(
            "If `True`, the Nebula container will be configured to send its output "
    ...
    stream_output: bool = Field(
        default=None,
        description=(
            "If `True`, logs will be streamed from the Nebula container to the local "
    ...
    env: Dict[str, Optional[str]] = Field(
        ...
        description=(
            ...
            "on the Nebula container at task runtime. These will not be set on the "
            "task definition."
        ),
    )
    ...
    def image_is_required(cls, values: dict) -> dict:
        ...
        nebula_container = (
            get_nebula_container(
                (values.get("task_definition") or {}).get("containerDefinitions", [])
            )
            or {}
        )
        image_in_task_definition = nebula_container.get("image")

        # If a task_definition is given with a nebula container image, use that value
        if image_in_task_definition:
            values["image"] = image_in_task_definition
        # Otherwise, it should default to the Nebula base image
        else:
            values["image"] = get_nebula_image_name()
        return values
    ...
    async def run(self, task_status: Optional[TaskStatus] = None) -> ECSTaskResult:
        ...
        command = self.command or get_nebula_container(
            task_definition["containerDefinitions"]
        ).get("command", [])
        self.logger.info(
            f"{self._log_prefix}: Running command {' '.join(command)!r} "
            f"in container {NEBULA_ECS_CONTAINER_NAME!r} ({self.image})..."
        )
        ...
        # Note "::" is used despite the Nebula standard being ":" because ARNs contain
        # single colons.
    ...
    def _watch_task_and_get_exit_code(
        ...
        """
        Wait for the task run to complete and retrieve the exit code of the Nebula
        container.
        """
        ...
        # Check the status code of the Nebula container
        nebula_container = get_nebula_container(task["containers"])
        assert (
            nebula_container is not None
        ), f"'nebula' container missing from task: {task}"
        status_code = nebula_container.get("exitCode")
        self._report_container_status_code(NEBULA_ECS_CONTAINER_NAME, status_code)
    ...
    def _wait_for_task_finish(
        ...
        If configured, logs from the Nebula container are streamed to stderr.
        ...
        if self.stream_output:
            container_def = get_nebula_container(
                task_definition["containerDefinitions"]
            )
            if not container_def:
                self.logger.warning(
                    f"{self._log_prefix}: Nebula container definition not found in "
                ...
                stream_name = "/".join(
                    [
                        ...
                        NEBULA_ECS_CONTAINER_NAME,
                        task_arn.rsplit("/")[-1],
                    ]
                )
                self.logger.info(
                    f"{self._log_prefix}: Streaming output from container "
                    f"{NEBULA_ECS_CONTAINER_NAME!r}..."
                )
    ...
    def _stream_available_logs(
        ...
        while last_log_stream_token != next_log_stream_token:
            ...
            for log_event in log_events:
                ...
                #       when the content is output from a Nebula logger on the
    ...
    def _prepare_task_definition(self, task_definition: dict, region: str) -> dict:
        ...
        # Configure the Nebula runtime container
        task_definition.setdefault("containerDefinitions", [])
        container = get_nebula_container(task_definition["containerDefinitions"])
        if container is None:
            container = {"name": NEBULA_ECS_CONTAINER_NAME}
        ...
        if self.configure_cloudwatch_logs:
            container["logConfiguration"] = {
                ...
                "options": {
                    ...
                    "awslogs-group": "nebula",
                    "awslogs-region": region,
                    "awslogs-stream-prefix": self.name or "nebula",
    ...
    def _prepare_task_run_overrides(self) -> dict:
        ...
        overrides = {
            "containerOverrides": [
                {
                    "name": NEBULA_ECS_CONTAINER_NAME,
                    "environment": [
        ...
        nebula_container_overrides = overrides["containerOverrides"][0]

        if self.command:
            nebula_container_overrides["command"] = self.command
        ...
        if self.memory:
            ...
            nebula_container_overrides.setdefault("memory", self.memory)

        if self.cpu:
            overrides["cpu"] = str(self.cpu)
            nebula_container_overrides.setdefault("cpu", self.cpu)
