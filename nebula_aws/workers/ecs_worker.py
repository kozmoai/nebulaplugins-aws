"""
Nebula worker for executing flow runs as ECS tasks.
...
```
$ nebula worker start --pool my-ecs-pool
```
...
from nebula.workers.base import (
...
from nebula_aws.credentials import AwsCredentials, ClientType

# Internal type alias for ECS clients which are generated dynamically in botocore
_ECSClient = Any

ECS_DEFAULT_CONTAINER_NAME = "nebula"
ECS_DEFAULT_CPU = 1024
ECS_DEFAULT_COMMAND = "python -m nebula.engine"
...
def mask_api_key(task_run_request):
    return mask_sensitive_env_values(
        deepcopy(task_run_request), ["NEBULA_API_KEY"], keep_length=6
    )
...
class ECSVariables(BaseVariables):
    ...
    configure_cloudwatch_logs: bool = Field(
        default=None,
        description=(
            "If enabled, the Nebula container will be configured to send its output "
    ...
    stream_output: bool = Field(
        default=None,
        description=(
            "If enabled, logs will be streamed from the Nebula container to the local "
...
class ECSWorker(BaseWorker):
    """
    A Nebula worker to run flow runs as ECS tasks.
    """
    ...
    _documentation_url = "https://kozmoai.github.io/nebulaplugins-aws/ecs_worker/"
    ...
    def _wait_for_task_finish(
        ...
        if configuration.stream_output:
            ...
            if not container_def:
                logger.warning(
                    "Nebula container definition not found in "
    ...
    def _prepare_task_definition(
        ...
        container.setdefault("image", get_nebula_image_name())
        ...
        if configuration.configure_cloudwatch_logs:
            container["logConfiguration"] = {
                ...
                "options": {
                    ...
                    "awslogs-stream-prefix": configuration.name or "nebula",
